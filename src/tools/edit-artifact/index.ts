...
export const EditArtifactParamsSchema = z.object({
  path: z.string().describe('Relative path for the artifact inside .chuck/artifacts/ (e.g. "src/engine.ts")'),
...
export const editArtifactTool: ToolDefinition<typeof EditArtifactParamsSchema, EditArtifactResult> = {
  ...
  icon: '✎',
  description: 'Edits an existing artifact file inside .chuck/artifacts/ by replacing an exact unique occurrence of old_string with new_string. Never edits the host project.',
  ...
  async execute(args, context): Promise<EditArtifactResult> {
    ...
    const updatedContent =
      currentContent.slice(0, firstIndex) + new_string + currentContent.slice(firstIndex + old_string.length);
