...
export class TuiAltScreen extends TuiBase implements ViewportTUI {
  ...
  constructor(
    ...
    this.searchCurrentMatchStyle =
      options.searchCurrentMatchStyle ?? ((text) => `\x1b[1;7m${text}\x1b[22;27m`);
  ...
  protected override afterTerminalStop(options: TuiStopOptions): void {
    ...
    if (options.preserveScreen) {
      ...
      ).map((line) => (visibleWidth(line) <= width ? line : sliceByColumn(line, 0, width, true)));
  ...
  private scrollToPrompt(direction: -1 | 1): void {
    ...
    for (
      let row = scrollView.scrollTop + direction;
  ...
  private getSearchNavigationDirectionAt(x: number, y: number): -1 | 1 | undefined {
    ...
    if (
      x < bounds.col ||
      x >= bounds.col + bounds.width ||
      y < bounds.row ||
      y >= bounds.row + bounds.height
    ) {
      return undefined;
    }
  ...
  private refreshSearch(layout: LayoutFrame): boolean {
    ...
    if (matches.length > 0) {
      if (search.selectionMode === 'query') {
        ...
        const baseIndex =
          exactIndex >= 0 ? exactIndex : Math.min(search.selectedIndex, matches.length - 1);
        selectedIndex =
          baseIndex < 0 ? matches.length - 1 : (baseIndex - 1 + matches.length) % matches.length;
      } else {
        selectedIndex =
          exactIndex >= 0
            ? exactIndex
    ...
    search.selectedKey =
      selectedIndex >= 0 ? getAltScreenSearchMatchKey(matches[selectedIndex]!) : undefined;
    search.selectionMode = 'retain';
  ...
  private handleViewportInput(data: string): { consume?: boolean } | undefined {
    if (data === FOCUS_OUT) {
      ...
      const hadNonEmptyActiveSelection =
        hadActiveSelection && this.getSelectionBounds() !== undefined;
      this.selectionPressActive = false;
      this.stopSelectionAutoScroll();
      this.stopScrollbarHover();
    ...
    if (wheelEvent) {
      ...
      const result =
        overlay.result ?? (overlay.hit ? undefined : this.dispatchMouseToLayout(event));
      if (result) {
  ...
  private dispatchMouseToLayout(event: TuiMouseEvent): TuiMouseDispatchResult | undefined {
    ...
    for (const box of boxes) {
      ...
      if (
        getLayoutNode(box.component) &&
        box.component.handleMouse === Container.prototype.handleMouse
      )
        continue;
  ...
  private handleMouseEvent(raw: SgrMouseEvent): void {
    ...
    if (this.mouseCapture || this.mousePressTarget) {
      ...
      if (
        ...
        (raw.x !== this.mousePressPoint.x || raw.y !== this.mousePressPoint.y)
      ) {
  ...
  private getWheelScrollLines(button: number): number {
    ...
    return (button & 8) !== 0
      ? this.wheelScrollLines * ALT_WHEEL_SCROLL_MULTIPLIER
  ...
  private scrollScrollbarToPointer(
    ...
    const thumbOffset = Math.max(
      0,
      Math.min(maxThumbOffset, pointerY - geometry.trackTop - grabOffset),
    );
    const scrollTop =
  ...
  private handleScrollbarMouseEvent(event: SgrMouseEvent): boolean {
    ...
    const grabOffset = onThumb
      ? event.y - target.geometry.thumbTop
      : Math.floor(target.geometry.thumbHeight / 2);
    if (!onThumb)
  ...
  private getWordSelection(point: SelectionPoint): SelectionRange | undefined {
    ...
    for (
      ...
      index > 0 && canJoin(segments[index - 1], segments[index]);
      index--
    ) {
  ...
  private updateSelectionAutoScroll(event: SgrMouseEvent): void {
    ...
    this.selectionAutoScrollDirection =
      event.y <= visibleTop ? -1 : event.y >= visibleBottom ? 1 : 0;
  ...
  private handleSelectionMouseEvent(event: SgrMouseEvent): void {
    ...
    if (event.release) {
      ...
      if (isClick) {
        ...
        const result =
          overlay.result ?? (overlay.hit ? undefined : this.dispatchMouseToLayout(clickEvent));
        if (result) {
  ...
  private isMouseSequence(data: string): boolean {
    return (
      /^\x1b\[<\d+;\d+;\d+[Mm]$/.test(data) || (data.length === 6 && data.startsWith('\x1b[M'))
    );
  }
