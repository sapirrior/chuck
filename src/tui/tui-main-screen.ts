...
export class TuiMainScreen extends TuiBase implements TUI {
  ...
  protected doRender(): void {
    ...
    const redrawLogDirectory =
      process.env.XD_TUI_DEBUG_REDRAW === '1' ? this.logDirectory : undefined;
    const logRedraw = (reason: string): void => {
      if (redrawLogDirectory === undefined) return;
      const logPath = path.join(redrawLogDirectory, 'xd-tui-debug.log');
    ...
    for (let i = firstChanged; i <= renderEnd; i++) {
      ...
      if (visibleWidth(line) > width) {
        ...
        const crashLogPath = path.join(this.logDirectory ?? os.tmpdir(), 'xd-tui-crash.log');
        const crashData = [
