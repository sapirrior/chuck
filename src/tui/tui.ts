...
export function retargetMouseEvent(
  event: TuiMouseEvent,
  target: TuiMouseDispatchTarget,
): TuiMouseEvent {
  return {
...
type OverlayBlockedFocusResume =
  { status: 'restore-overlay' } | { status: 'focus-target'; target: Component | null };
...
type ActiveOverlayFocusRestoreState =
  EligibleOverlayFocusRestoreState | BlockedOverlayFocusRestoreState;
  ...
  handleMouse(event: TuiMouseEvent): TuiMouseDispatchResult | undefined {
    ...
    for (const { component: child, height: childHeight } of mouseChildren) {
      if (event.y >= childY && event.y < childY + childHeight) {
        ...
        if (result?.focus && (this as Component).handleInput)
...
export const VIEWPORT_TUI = Symbol.for('xd/tui/viewport');
