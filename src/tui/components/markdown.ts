...
export class Markdown implements Component {
  ...
  render(width: number): string[] {
    // Return cached result if text and width haven't changed
    ...
    const text = this.options.transform
      ? this.options.transform(this.text, contentWidth)
      : this.text;

    // Normalize text: replace tabs with spaces (3 spaces per tab)
