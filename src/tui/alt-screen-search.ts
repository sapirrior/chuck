  ...
  for (const match of corpus.text.matchAll(expression)) {
    ...
    for (let index = spanIndex; index < corpus.spans.length; index++) {
      ...
      const startCol = span.linearColumns
        ...
        : span.startCol;
      const endCol = span.linearColumns
        ? span.startCol + Math.min(end, span.textEnd) - span.textStart
        : span.endCol;
...
export function findAltScreenSearchMatches(
...
export class AltScreenSearchComponent implements Component, Focusable {
  ...
  render(width: number): string[] {
    ...
    const inputLine = truncateToWidth(
      this.input.render(Math.max(1, inputWidth))[0] ?? '',
      inputWidth,
      '',
    );
    ...
    let controlsWidth =
      visibleWidth(previousButton) + visibleWidth(separator) + visibleWidth(nextButton);
