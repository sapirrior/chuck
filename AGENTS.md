...
Universal operational guidelines for AI coding agents working in the `chuck` repository.
...
- **Tools (`src/tools/`):** Action and tool definitions exposed to the model (`write_artifact`, `edit_artifact`, `write_plan`, `read_file`, `find_files`, `search_text`, `list_dir`, `web_fetch`, `web_search`). Artifacts and plans are strictly confined to `.chuck/`.
...
| `bun run compile` | Compile to a standalone binary `./dist/chuck` |
...
- **Non-Destructive Philosophy:** Host workspace files are not directly edited by artifact tools; all artifact writes/edits are confined to `.chuck/`.
